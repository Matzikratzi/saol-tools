...
def _source() -> str:
    ...
    new_globals = (
        ...
        "_RIGHT_HAF_LEVELS: tuple[float, float, float] | None = None\n"
        "_RIGHT_THRESHOLD_X: float | None = None\n"
        "_DESKEWED_CONTENT: bytes | None = None\n"
        "_SYNTHETIC_ARTICLE_ROWS: list[str] = []\n"
    )
...
'''
    ...
    new_result_start = (
        "    global _LEFT_A_X, _LEFT_LEVELS, _LEFT_LEVEL_COUNTS, _LEFT_IGNORED_HEADINGS, _LEFT_PREFIX_PAIRS, _HAF_LEVELS, _LEFT_THRESHOLD_X, _RIGHT_HAF_LEVELS, _RIGHT_THRESHOLD_X, _SYNTHETIC_ARTICLE_ROWS\n"
        ...
        "        else None\n"
        "    )\n"
        "    # Measure the right column independently. Most pages have no visible\n"
        "    # homonym number there, so H may have to be inferred 23 px left of A.\n"
        "    right_starts = sorted(float(line.raw_start_x) for line, *_rest in raw[2])\n"
        "    right_clusters = []\n"
        "    for value in right_starts:\n"
        "        if right_clusters and value - right_clusters[-1][-1] <= tolerance:\n"
        "            right_clusters[-1].append(value)\n"
        "        else:\n"
        "            right_clusters.append([value])\n"
        "    right_minimum_count = max(2, round(len(right_starts) * 0.04))\n"
        "    right_levels = [\n"
        "        float(statistics.median(cluster)) for cluster in right_clusters\n"
        "        if len(cluster) >= right_minimum_count\n"
        "    ][:4]\n"
        "    right_matches = []\n"
        "    for first in range(len(right_levels) - 2):\n"
        "        for second in range(first + 1, len(right_levels) - 1):\n"
        "            for third in range(second + 1, len(right_levels)):\n"
        "                h, a, f = right_levels[first], right_levels[second], right_levels[third]\n"
        "                if abs((a - h) - 23.0) <= 8.0 and abs((f - a) - 40.0) <= 10.0:\n"
        "                    right_matches.append((abs((a - h) - 23.0) + abs((f - a) - 40.0), h, a, f))\n"
        "    if right_matches:\n"
        "        _RIGHT_HAF_LEVELS = tuple(min(right_matches)[1:])\n"
        "    else:\n"
        "        right_pairs = [\n"
        "            (abs((f - a) - 40.0), a, f)\n"
        "            for index, a in enumerate(right_levels)\n"
        "            for f in right_levels[index + 1:]\n"
        "            if abs((f - a) - 40.0) <= 10.0\n"
        "        ]\n"
        "        _RIGHT_HAF_LEVELS = (\n"
        "            (min(right_pairs)[1] - 23.0, min(right_pairs)[1], min(right_pairs)[2])\n"
        "            if right_pairs else None\n"
        "        )\n"
        "    _RIGHT_THRESHOLD_X = (\n"
        "        (_RIGHT_HAF_LEVELS[1] + _RIGHT_HAF_LEVELS[2]) / 2\n"
        "        if _RIGHT_HAF_LEVELS is not None else None\n"
        "    )\n"
        "    _SYNTHETIC_ARTICLE_ROWS = []\n"
        "    if _HAF_LEVELS is not None and _DESKEWED_CONTENT is not None and raw[1]:\n"
        "        page_image = Image.open(io.BytesIO(_DESKEWED_CONTENT)).convert('L')\n"
        "        pixels = page_image.load()\n"
        "        scan_x0 = max(0, round(_HAF_LEVELS[0] - median_height * 0.30))\n"
        "        scan_x1 = min(page_image.width, round(_LEFT_THRESHOLD_X))\n"
        "        active_rows = []\n"
        "        for y in range(max(0, round(body_top)), min(page_image.height, round(image_height * 0.94))):\n"
        "            dark = sum(1 for x in range(scan_x0, scan_x1) if pixels[x, y] < 160)\n"
        "            if dark >= 2:\n"
        "                active_rows.append(y)\n"
        "        bands = []\n"
        "        if active_rows:\n"
        "            band = [active_rows[0]]\n"
        "            max_gap = max(2, round(median_height * 0.12))\n"
        "            for y in active_rows[1:]:\n"
        "                if y - band[-1] <= max_gap:\n"
        "                    band.append(y)\n"
        "                else:\n"
        "                    bands.append((band[0], band[-1] + 1))\n"
        "                    band = [y]\n"
        "            bands.append((band[0], band[-1] + 1))\n"
        "        existing = [entry[0] for entry in raw[1]]\n"
        "        template = raw[1][0][2]\n"
        "        for band_top, band_bottom in bands:\n"
        "            if band_bottom - band_top < max(3, median_height * 0.12):\n"
        "                continue\n"
        ...
        "                continue\n"
        "            crop_left = scan_x0\n"
        "            crop_right = min(round(split), round(_HAF_LEVELS[1] + median_height * 12.0))\n"
        "            crop_top = max(0, round(band_top - median_height * 0.25))\n"
        "            crop_bottom = min(page_image.height, round(band_bottom + median_height * 0.25))\n"
        "            crop = page_image.crop((crop_left, crop_top, crop_right, crop_bottom))\n"
        "            buffer = io.BytesIO()\n"
        "            crop.save(buffer, format='PNG')\n"
        "            try:\n"
        "                recovered = module.extract_observations(buffer.getvalue())\n"
        "            except Exception:\n"
        "                recovered = []\n"
        "            recovered_items = tuple(sorted((\n"
        "                replace(item, left=item.left + crop_left, top=item.top + crop_top)\n"
        "                for item in recovered if LETTER_RE.search(item.text)\n"
        ...
        "            else:\n"
        "                word = replace(\n"
        "                    template, text='oläst', ocr_tesseract='oläst',\n"
        "                    left=round(_HAF_LEVELS[1]), top=band_top,\n"
        "                    width=max(1, round(median_height * 2.5)),\n"
        "                    height=max(1, band_bottom - band_top),\n"
        "                )\n"
        "                items = (word,)\n"
        "                word_x, marker_x, word_text = float(word.left), None, word.text\n"
        "            left = min(item.left for item in items)\n"
        "            right = min(split, max(item.left + item.width for item in items))\n"
        "            top = min(item.top for item in items)\n"
        "            bottom = max(item.top + item.height for item in items)\n"
        "            text = ' '.join(item.text for item in items)\n"
        "            line = module.PrintedLine(\n"
        "                column=1, items=items, text=text, first=word,\n"
        "                left=left, top=top, right=right, bottom=bottom,\n"
        "                raw_start_x=float(left), letter_start_x=word_x,\n"
        "                has_homonym_marker=marker_x is not None,\n"
        "            )\n"
        "            raw[1].append((line, word_x, word, marker_x, word_text))\n"
        "            existing.append(line)\n"
        "            _SYNTHETIC_ARTICLE_ROWS.append(f'{band_top:.0f}:{word_text}')\n"
        "        page_image.close()\n"
        "\n"
        "    result = []\n"
        "    models = {}\n"
    ...
    new_positions = (
        ...
        "        elif column == 2 and _RIGHT_HAF_LEVELS is not None:\n"
        "            _h_x, article_x, continuation_x = _RIGHT_HAF_LEVELS\n"
    ...
    new_article_decision = (
        "            threshold_x = (\n"
        "                _LEFT_THRESHOLD_X if column == 1 else _RIGHT_THRESHOLD_X\n"
        "            )\n"
        ...
        "                is_article = any(\n"
        "                    LETTER_RE.search(item.text) and item.left < threshold_x\n"
    ...
    new_guides = (
        ...
        "        )\n"
        "    for haf_levels, threshold_x in (\n"
        "        (_HAF_LEVELS, _LEFT_THRESHOLD_X),\n"
        "        (_RIGHT_HAF_LEVELS, _RIGHT_THRESHOLD_X),\n"
        "    ):\n"
        "        if haf_levels is not None:\n"
        "            for index, (label, color, position) in enumerate(zip(('H', 'A', 'F'), ('#7c3aed', '#16a34a', '#ea580c'), haf_levels)):\n"
        ...
        "                )\n"
        "        if threshold_x is not None:\n"
        "            x = max(0.0, min(float(image_width), float(threshold_x)))\n"
        "            result.append(\n"
    ...
    new_guide_loop = (
        "        for kind, color in definitions:\n"
        "            # Both columns use the page-wide geometrical H/A/F/T guides.\n"
        "            if column in (1, 2):\n"
        "                continue\n"
        "            position = positions[kind]\n"
    )
...
'''
    ...
    new_console_summary = (
        ...
        "    right_haf = '–' if _RIGHT_HAF_LEVELS is None else '/'.join(f'{item:.1f}' for item in _RIGHT_HAF_LEVELS)\n"
        "    right_threshold = value(_RIGHT_THRESHOLD_X)\n"
        "    synthetic = ','.join(_SYNTHETIC_ARTICLE_ROWS) or '–'\n"
        "    print(\n"
        ...
        "        f'vänster-H/A/F={haf} vänster-T={threshold} '\n"
        "        f'höger-H/A/F={right_haf} höger-T={right_threshold} '\n"
        "        f'bildrader={synthetic}'\n"
        "    )\n"
    )
