...
def _layout_candidates(observations: list, image_width: int, image_height: int) -> tuple[list[LayoutCandidate], dict[int, float]]:
    """Find likely article starts using the fixed two-column SAOL layout.

    A candidate must be the first OCR token on a line, sit close to the common
    left margin of its own column and look darker than ordinary body text.
    """
    ...
    for column in (1, 2):
        ...
        # Headwords form the strong, non-indented cluster at the low end.
        ...
        bold_reference = _quantile(inks, 0.72) if inks else ordinary_ink
        bold_reference = max(bold_reference, ordinary_ink, 1e-6)
        ...
        for item in starts:
            ...
            # Layout dominates. Darkness/boldness confirms the flush-left start.
    ...
    *,
    ...
    for candidate in sorted(displayed, key=lambda value: (value.column, value.item.top, value.item.left)):
        ...
        rows.append(
            ...
            '<td>%d</td><td>%s</td><td><strong>%s</strong></td><td>%s</td><td>%s</td>'
            '<td>%.0f%%</td><td>%.1f</td><td>%.2f</td><td>%.1f</td></tr>'
            % (
                css_class,
                item.left,
                item.top,
    ...
    mode = "all OCR-radstarter" if all_ocr else f"artikelstarter ≥ {threshold:.0%}"
    heading = "Alla radstarter i två spalter" if all_ocr else "Troliga grundord / artikelstarter"
    empty_message = '<tr><td colspan="9">Inga artikelstarter hittades över vald tröskel.</td></tr>'
...
.note {{ margin-top: 10px; font-size: .9rem; opacity: .9; }}
...
.image-wrap {{ height: calc(100vh - 225px); min-height: 420px; overflow: auto; padding: 12px; background: #d1d5db; }}
...
.table-wrap {{ height: calc(100vh - 165px); min-height: 480px; overflow: auto; }}
...
tr.accepted {{ background: #dcfce7; }} tr.conflict {{ background: #fee2e2; }} tr.tesseract-only {{ background: #fff; }}
...
@media (prefers-color-scheme: dark) {{
 ...
 .image-wrap {{ background: #111827; }} tr.accepted {{ background: #14532d; }} tr.conflict {{ background: #7f1d1d; }} tr.tesseract-only {{ background: #1f2937; }}
}}
</style>
</head>
<body>
<header>
  <h1>Grundordsgranskning – sida {page}</h1>
  <div class="summary">
    <span class="badge">Läge: {_escape(mode)}</span>
    <span class="badge">Spalt 1-marginal: {margins.get(1, 0):.0f}px</span>
    <span class="badge">Spalt 2-marginal: {margins.get(2, 0):.0f}px</span>
    <span class="badge">Kandidater: {len(displayed)}</span>
    <span class="badge good">OCR-eniga: {len(accepted)}</span>
    <span class="badge bad">OCR-konflikter: {len(conflicts)}</span>
    <span class="badge">Radsimilaritet: {mean_similarity:.3f}</span>
  </div>
  <div class="note">Urvalet bygger först på två spalter, vänstermarginal och radstart; mörk/fet text används som bekräftelse.</div>
</header>
<main>
  <div class="grid">
    <section class="panel">
      ...
      <div class="image-toolbar">
        <button type="button" onclick="changeZoom(-0.25)">−</button><button type="button" onclick="fitImage()">Anpassa</button><button type="button" onclick="changeZoom(0.25)">+</button>
        <span id="zoomLabel">100 %</span><span class="hint">Klicka på ett grundord för att hitta det</span>
      </div>
      <div class="image-wrap" id="imageWrap"><div class="scan-stage" id="scanStage"><img id="scanImage" src="{image_data}" alt="Skannad SAOL-sida {page}"><div class="marker" id="marker"></div></div></div>
    </section>
    <section class="panel">
      <h2>{heading}</h2>
      <div class="table-wrap"><table>
        <thead><tr><th>Spalt</th><th>Status</th><th>Grundord</th><th>Tesseract</th><th>Runeberg</th><th>Layout</th><th>Indrag</th><th>Svärta</th><th>OCR</th></tr></thead>
        <tbody>{''.join(rows) or empty_message}</tbody>
      </table></div>
    </section>
  </div>
  <details><summary>Debug: matchade OCR-rader ({len(pairs)})</summary><div class="panel table-wrap"><table><thead><tr><th>T-rad</th><th>R-rad</th><th>Likhet</th><th>Tesseract</th><th>Runeberg</th></tr></thead><tbody>{''.join(line_rows)}</tbody></table></div></details>
...
const imageWrap=document.getElementById('imageWrap'), scanStage=document.getElementById('scanStage'), marker=document.getElementById('marker'), zoomLabel=document.getElementById('zoomLabel');
let zoom=1, selectedRow=null;
function setZoom(value, keepCenter=true) {{ const old=zoom, cx=(imageWrap.scrollLeft+imageWrap.clientWidth/2)/old, cy=(imageWrap.scrollTop+imageWrap.clientHeight/2)/old; zoom=Math.max(.15,Math.min(3,value)); scanStage.style.width=`${{naturalWidth*zoom}}px`; scanStage.style.height=`${{naturalHeight*zoom}}px`; scanImage.style.width=`${{naturalWidth*zoom}}px`; scanImage.style.height=`${{naturalHeight*zoom}}px`; zoomLabel.textContent=`${{Math.round(zoom*100)}} %`; if(selectedRow) positionMarker(selectedRow); if(keepCenter){{imageWrap.scrollLeft=cx*zoom-imageWrap.clientWidth/2;imageWrap.scrollTop=cy*zoom-imageWrap.clientHeight/2;}} }}
function fitImage() {{ setZoom(Math.min(1,Math.max(100,imageWrap.clientWidth-24)/naturalWidth),false); imageWrap.scrollTo(0,0); }}
function changeZoom(delta) {{ setZoom(zoom+delta); }}
function positionMarker(row) {{ const l=+row.dataset.left,t=+row.dataset.top,w=+row.dataset.width,h=+row.dataset.height,m=Math.max(4,h*.25); marker.style.left=`${{(l-m)*zoom}}px`; marker.style.top=`${{(t-m)*zoom}}px`; marker.style.width=`${{(w+2*m)*zoom}}px`; marker.style.height=`${{(h+2*m)*zoom}}px`; marker.style.display='block'; }}
function focusObservation(row) {{ if(selectedRow)selectedRow.classList.remove('selected'); selectedRow=row; row.classList.add('selected'); if(zoom<.8)setZoom(1,false); positionMarker(row); const l=+row.dataset.left*zoom,t=+row.dataset.top*zoom,w=+row.dataset.width*zoom,h=+row.dataset.height*zoom; imageWrap.scrollTo({{left:Math.max(0,l+w/2-imageWrap.clientWidth/2),top:Math.max(0,t+h/2-imageWrap.clientHeight/2),behavior:'smooth'}}); }}
function activateRow(event,row) {{ if(event.key==='Enter'||event.key===' '){{event.preventDefault();focusObservation(row);}} }}
window.addEventListener('load',fitImage); window.addEventListener('resize',()=>{{if(!selectedRow)fitImage();}});
</script>
</body>
</html>
"""
...
def main() -> None:
    ...
    parser.add_argument("--threshold", type=float, default=0.55, help="Lägsta layoutpoäng för artikelstart (standard: 0.55).")
    ...
    candidates = [
        LayoutCandidate(
            corrected_by_position.get((candidate.item.left, candidate.item.top, candidate.item.width, candidate.item.height), candidate.item),
            candidate.column,
            candidate.margin,
            candidate.indent,
            candidate.ink_ratio,
